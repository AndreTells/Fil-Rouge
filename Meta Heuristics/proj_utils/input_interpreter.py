...
def _calculate_adjacency_matrix(route_id, customer_df, deposit_df ):
...
def _calculate_delivery_windows(route_id, customer_df):
    ...
    list_window_start = df_delivery_window['CUSTOMER_TIME_WINDOW_FROM_MIN'].tolist()
    list_window_end = df_delivery_window['CUSTOMER_TIME_WINDOW_TO_MIN'].tolist()
    ...
    #res = [(0,float('inf'))] +list(zip(list_window_start, list_window_end))

    return res

def format_input(route_id, customer_df, deposit_df):
    customer_loads = _calculate_customer_loads(route_id, customer_df)
    adjacency_matrix,points = _calculate_adjacency_matrix(route_id, customer_df, deposit_df )
